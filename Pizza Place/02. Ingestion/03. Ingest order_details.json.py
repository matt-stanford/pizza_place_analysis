...
file_path = 'abfss://bronze@dbpizzaplacesa.dfs.core.windows.net/order_details'
table_name = 'order_details_bronze'
...
def upsert_to_delta(input_df, batch_id):
    if spark._jsparkSession.catalog().tableExists(f'pizza_place_bronze.{table_name}'):
        ...
        (delta_df.alias('t')
            .merge(input_df.alias('s'), 't.order_details_id = s.order_details_id')
...
# MAGIC %sql
# MAGIC SELECT * FROM pizza_place_bronze.order_details_bronze
